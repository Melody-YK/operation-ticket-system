...
graph TB
    ...
    subgraph MVP["MVP 版本"]
        direction LR
        S11[US-001a] --> S21[US-003a] --> S31[US-006] --> S41[US-007a] --> S51[US-011] --> S61[US-014]
        S12[US-002] --> S22[US-004]
        S23[US-003b] --> S42[US-007b]
    end

    subgraph V11["V1.1 版本"]
        direction LR
        S13[US-001b] --> S24[US-005] --> S43[US-008] --> S52[US-012] --> S62[US-015]
        S14[US-001c] --> S44[US-007c] --> S71[US-018]
        S25[US-017]
    end

    subgraph V20["V2.0 版本"]
        direction LR
        S45[US-009] --> S53[US-013] --> S63[US-016]
...
|------|-----------|------------|------------|
| **① 建票送审** | US-001a 建票（基础信息+操作内容+自动保存）, US-002 送审 | US-001b 危险源/工器具管理, US-001c 工作票关联, US-017 暂存续编 | - |
| **② 三级审核** | US-003a 监护人审核通过/驳回, US-003b 审核编辑, US-004 批准人审核 | US-005 发令人审核 | - |
| **③ 指令下达** | US-006 指令下达与锁定 | - | - |
| **④ 现场执行** | US-007a 逐条执行与状态标记, US-007b 数据实时上传 | US-007c 监护人监控, US-008 断点续传 | US-009 操作中止 |
...
**故事**：
> 作为一名**操作人**，
> 我希望在**PC/移动端（同一套响应式Web系统）**创建电子操作票，填写基础信息和操作内容，
...
**故事**：
> 作为一名**操作人**，
> 我希望**在创建操作票时关联危险源和工器具信息**，
> 以便**确保操作票包含完整的安全防控和工具准备信息**。
...
Scenario: 添加危险源
  Given 操作人正在编辑操作票
  When 操作人添加危险源信息（类型、描述、防控措施）
  Then 系统保存危险源信息并关联到操作票

Scenario: 添加工器具
  Given 操作人正在编辑操作票
  When 操作人添加工器具信息（名称、型号、数量）
  Then 系统保存工器具信息并关联到操作票
```

**INVEST 检查**：
- [ ] **I**ndependent（依赖 US-001a 创建操作票）
- [x] **N**egotiable（可协商）
- [x] **V**aluable（有价值）
- [x] **E**stimable（CRUD 操作，逻辑清晰）
- [x] **S**mall（两个独立子功能，范围小）
- [x] **T**estable（可测试）
...
**故事**：
> 作为一名**操作人**，
> 我希望**在创建操作票时输入工作票编号，系统自动关联安措内容**，
> 以便**减少手动录入，确保操作票与工作票信息一致**。
...
Scenario: 关联工作票
  Given 操作人正在编辑操作票
  When 操作人输入工作票编号
  Then 系统自动查询并关联该工作票的安措内容

Scenario: 工作票编号无效
  Given 操作人正在编辑操作票
  When 操作人输入不存在的工作票编号
  Then 系统提示"工作票不存在，请检查编号"
```
...
**故事**：
> 作为一名**监护人**，
> 我希望**审核操作人提交的操作票**，查看全量信息，审核通过或驳回，
...
**故事**：
> 作为一名**监护人**，
> 我希望**在审核过程中直接编辑修改操作票内容**，
> 以便**快速修正发现的小问题，无需退回给操作人修改**。
...
Scenario: 编辑后审核通过
  Given 监护人已编辑修改操作票
  When 监护人点击"审核通过"
  Then 系统提交编辑后的操作票至下一审核节点
```

**INVEST 检查**：
- [ ] **I**ndependent（依赖 US-003a 审核流程）
- [x] **N**egotiable（可协商）
- [x] **V**aluable（有价值）
- [x] **E**stimable（可估算）
- [x] **S**mall（单功能点）
...
**INVEST 检查**：
- [ ] **I**ndependent（依赖 US-007a 执行中状态）
- [x] **N**egotiable（上传策略可协商）
- [x] **V**aluable（有价值）
...
**INVEST 检查**：
- [ ] **I**ndependent（依赖 US-007a/US-007b 执行和数据上传）
- [x] **N**egotiable（监控刷新频率可协商）
- [x] **V**aluable（有价值）
- [x] **E**stimable（数据查询展示，逻辑清晰）
- [x] **S**mall（单功能——监控看板）
- [x] **T**estable（可测试）
...
sequenceDiagram
    ...
    Operator->>System: 1. 进入创建操作票页面
    System->>DB: 2. 生成唯一票号
    System-->>Operator: 3. 展示空白操作票表单

    loop 编辑过程
        Operator->>System: 4. 填写/修改表单字段
        System->>DB: 5. 自动实时保存（逐字段）
    end

    Operator->>System: 6. 提交送审
    System->>DB: 7. 校验必填项完整性
    System->>DB: 8. 保存最终版本，状态=建立
    System-->>Operator: 9. 提示"已提交送审"
```
...
**A1：暂存草稿**
- **触发条件**：操作人未填写完所有必填项
- **描述**：系统自动保存已填写的字段为草稿，操作票状态保持"草稿"，下次可继续编辑

**A2：意外关闭恢复**
- **触发条件**：操作人意外关闭页面或切换设备
- **描述**：重新打开后自动恢复上次编辑内容，不丢失已填写数据

#### 异常流程

**E1：自动保存失败**
- **触发条件**：网络异常导致字段保存失败
- **系统响应**：前端缓存最新修改，显示"保存中..."提示
- **恢复**：网络恢复后自动同步至服务器
...
sequenceDiagram
    ...
    Dispatcher->>System: 1. 打开待审核操作票
    System->>DB: 2. 查询操作票全量信息
    DB-->>System: 3. 返回数据
    System-->>Dispatcher: 4. 展示审核页面（含下达指令按钮）

    Dispatcher->>System: 5. 审核通过并下达指令
    System->>DB: 6. 写入下令时间
    System->>DB: 7. 锁定操作票所有信息（禁止编辑）
    System->>DB: 8. 操作票状态=待执行
    System-->>Dispatcher: 9. 提示"指令已下达"
    System-->>Operator: 10. 推送待执行通知
```
...
**A1：仅审核通过，暂不下达指令**
- **触发条件**：发令人认为操作票内容正确，但当前不是合适的下令时机
- **描述**：发令人点击"审核通过"但不点击"下达指令"，操作票状态保持"待审核（发令人）"，后续可再次进入页面下达指令

#### 异常流程

**E1：操作票已被其他发令人处理**
- **触发条件**：操作票状态已被其他发令人变更（如审核驳回）
- **系统响应**：当前发令人提交时提示"操作票状态已变更，请刷新"
- **恢复**：刷新页面查看最新状态
...
sequenceDiagram
    ...
    System->>System: 1. 操作执行完成，触发校验流程
    System->>DB: 2. 读取操作票全量数据
    System->>DB: 3. 读取现场终端上传数据
    System->>System: 4. 逐项比对数据一致性
    alt 全部一致
        System-->>Operator: 5a. 展示"校验通过"
        System->>DB: 6a. 允许进入归档步骤
    else 存在异常
        System-->>Operator: 5b. 展示"校验异常"及异常数据列表
        Operator->>System: 6b. 点击"重新上传"修复异常
        System->>DB: 7b. 接收重新上传的数据
        System->>System: 8b. 重新校验
    end
```
...
- **触发条件**：校验发现数据不一致
- **描述**：
  1. 系统标记异常数据项（如缺失的音视频片段、不一致的设备状态）
  2. 操作人查看异常列表，对异常项点击"重新上传"
  3. 上传完成后系统自动重新校验

#### 异常流程

**E1：校验超时**
- **触发条件**：全量数据量大，校验耗时超过预期
- **系统响应**：后台异步执行校验，前端显示校验进度条
- **恢复**：校验完成后通知操作人查看结果
...
sequenceDiagram
    ...
    System->>System: 1. 数据校验通过，触发归档流程
    System->>DB: 2. 读取操作票全量数据
    System->>System: 3. 结构化数据存入归档表
    System->>System: 4. 生成PDF文档
    System->>DB: 5. 操作票状态=已完成
    System->>DB: 6. 记录归档时间
```
...
**A1：PDF生成失败**
- **触发条件**：PDF生成服务异常
- **描述**：系统记录生成失败日志，结构化数据仍正常归档，PDF可手动重新生成

#### 异常流程

**E1：归档锁冲突**
- **触发条件**：同一操作票被重复触发归档
- **系统响应**：判断已归档则跳过，返回已有归档记录
...
sequenceDiagram
    ...
    Dispatcher->>System: 1. 打开已中止操作票
    System->>DB: 2. 查询操作票及执行进度
    DB-->>System: 3. 返回已执行/未执行操作列表
    System-->>Dispatcher: 4. 展示执行进度

    Dispatcher->>System: 5. 点击"创建副本"
    System->>DB: 6. 复制原票数据，生成新票号
    System->>DB: 7. 标记已执行/未执行内容
    System-->>Dispatcher: 8. 展示副本操作票（可编辑）

    Dispatcher->>System: 9. 补充新操作人、监护人、批准人信息
    System->>DB: 10. 保存副本信息
    System-->>Dispatcher: 11. 提示"副本创建成功"
```
...
2. 原票已执行/未执行内容已正确标记
3. 副本分配了新票号
4. 副本基本信息（操作人、监护人、批准人）已维护

#### 备选流程

**A1：取消创建副本**
- **触发条件**：发令人决定暂不创建副本
- **描述**：在创建确认环节点击"取消"，原票保持"中止"状态不变

#### 异常流程

**E1：原票状态已变更**
- **触发条件**：创建副本时原票状态已被其他操作变更
- **系统响应**：提示"操作票状态已变更，无法创建副本"，刷新后查看最新状态
...
sequenceDiagram
    ...
    participant System as ⚙️ 系统
    participant External as 🔌 三峡行云

    System->>System: 1. 数据校验通过，触发推送流程
    System->>System: 2. 组装推送数据（操作结果+影像+安措内容）
    System->>External: 3. 调用三峡行云推送接口
    alt 推送成功
        External-->>System: 4a. 返回成功响应
        System->>System: 5a. 记录推送状态=成功
    else 推送失败
        External-->>System: 4b. 返回失败响应
        System->>System: 5b. 记录推送状态=失败
        System->>System: 6b. 自动重试（最多3次，指数退避）
    end
```
...
**A1：手动重新推送**
- **触发条件**：自动推送失败且重试用尽
- **描述**：系统记录失败原因，支持操作人/发令人手动点击"重新推送"触发重试

#### 异常流程

**E1：三峡行云接口不可用**
- **触发条件**：外部系统故障或网络不通
- **系统响应**：推送请求加入失败队列，标记为"待重推"
- **恢复**：外部系统恢复后，手动或定时任务触发重推
...
sequenceDiagram
    ...
    Operator->>System: 1. 提交作废申请并填写原因
    System->>DB: 2. 记录作废申请
    System-->>Dispatcher: 3. 推送作废审批待办

    Dispatcher->>System: 4. 打开作废审批页面
    System->>DB: 5. 查询操作票信息
    DB-->>System: 6. 返回操作票数据
    System-->>Dispatcher: 7. 展示申请及操作票信息

    alt 审批通过
        Dispatcher->>System: 8a. 审批通过
        System->>DB: 9a. 操作票状态=作废
        System->>DB: 10a. 记录作废人、作废时间、作废原因
        System->>DB: 11a. 作废票归档
        System-->>Operator: 12a. 通知"作废已批准"
    else 拒绝作废
        Dispatcher->>System: 8b. 拒绝并填写理由
        System->>DB: 9b. 操作票恢复原状态
        System-->>Operator: 10b. 通知"作废申请被拒绝"
    end
```

#### 前置条件
1. 操作票处于"建立"或"待审核"状态（未执行）
2. 操作人已登录系统
...
**E1：操作票已执行**
- **触发条件**：操作人尝试作废已执行的（状态≥"待执行"）操作票
- **系统响应**：提示"已执行的操作票不可作废，如需处理请使用操作中止流程"
...
|--------|---------|------|------|--------|----------|
| **SC-001** | US-001a | 功能性 | 操作票创建成功率 | ≥ 99.5% | 自动化测试 |
| **SC-002** | US-002 | 功能性 | 送审后待办推送成功率 | 100% | 系统日志 |
| **SC-003** | US-003a, US-003b | 性能 | 审核页面加载时间 | ≤ 2秒 | 性能测试 |
| **SC-004** | US-006 | 功能性 | 命令下达后信息锁定率 | 100% | 校验测试 |
| **SC-005** | US-007b | 功能性 | 现场数据实时上传成功率 | ≥ 99% | 数据一致性校验 |
...
graph LR
    subgraph Requirements["用户故事"]
        US001a[US-001a<br/>创建操作票]
        US002[US-002<br/>提交送审]
        US003a[US-003a<br/>监护人审核]
        US003b[US-003b<br/>审核编辑]
        US005[US-005<br/>发令人审核]
        US006[US-006<br/>指令下达]
        US007a[US-007a<br/>逐条执行]
        US007b[US-007b<br/>数据上传]
