...
- 性能压测、安全扫描、UAT 属于后续工程化阶段

## 7. Sprint 5 交付

Sprint 5 聚焦「测试 + 修复 + 交付收尾」：

### 后端测试

- 状态机单元测试覆盖：所有 16 条有效迁移路径、8 条非法迁移路径
- 28 个测试用例全部通过
- 后端 build + test：✅

### 前端验证

- 前端 build：✅ （TypeScript 编译 + Vite 打包）
- 前端 lint：0 errors，58 warnings（仅 `no-explicit-any` 设 warning 级别）

### 安全

- `.env` 已配置 `.gitignore` 排除
- `node_modules/`、`dist/`、`build/` 均不纳入版本管理

### 关键文档

- `README.md` — 项目概述、架构说明、快速启动、演示指南
- `QUICK_START.md` — 完整启动指南（Sprint 1-5）
- `USER_GUIDE.md` — 使用说明书（本文件）
- `IMPLEMENTATION_PLAN.md` — 实现计划（936 行，5 Sprint，48 任务）
- `PRD.md` — 原始需求文档

### 端到端流程

完整演示路径（覆盖登录 → 建票 → 提交 → 三级审核 → 下令 → 执行 → 校验）：

1. `zs` 登录 → 创建操作票 → 提交送审
2. `zl` 登录 → 审核通过（监护人）
3. `sb` 登录 → 审核通过（批准人）
4. `ws` 登录 → 审核通过并下达指令（发令人）
5. `zs` 登录 → 开始执行 → 逐项完成 → 提交校验
6. `ws` 登录 → 校验通过，归档
